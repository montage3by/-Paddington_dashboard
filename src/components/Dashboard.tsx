...
export function Dashboard() {
  ...
  const unmatchedLeads = useMemo(() => countUnmatchedLeads(dataset), []);
                ...
                <table className="w-full min-w-[640px] text-sm">
                  ...
                  <tbody>
                    ...
                    <tr className="border-b border-[var(--border)]">
                      ...
                      {periodComparison.map((p) => (
                        <td key={p.label} className="px-4 py-3 text-right text-[var(--text-primary)]">
                          {p.leads != null ? formatNumber(p.leads) : "—"}
                        </td>
                      ))}
                    </tr>
                    <tr>
                      <td className="px-4 py-3 text-[var(--text-primary)]">CPL</td>
                      {periodComparison.map((p) => (
                        <td key={p.label} className="px-4 py-3 text-right text-[var(--text-primary)]">
                          {p.cpl != null ? formatCurrency(p.cpl, currency) : "—"}
                        </td>
                      ))}
                    </tr>
                  </tbody>
        ...
        {tab === "campaigns" && (
          <div className="flex flex-col gap-6">
            <div>
              ...
              <CampaignsTable campaigns={campaigns} currency={currency} showLeads />
            </div>
            <div>
              <div className="mb-3 text-sm font-medium text-[var(--text-primary)]">
          ...
          <div className="flex flex-col gap-6">
            {unmatchedLeads > 0 && (
              <div className="rounded-lg border border-[#ba7517] bg-[#faeeda] px-3 py-2 text-xs text-[#854f0b]">
                {unmatchedLeads} лид(ов) вне рекламных кампаний (GBP / прямые заходы / не определено).
              </div>
            )}
            <div>
              <div className="mb-3 text-sm font-medium text-[var(--text-primary)]">
